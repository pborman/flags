...
// flag set, registering i with that set, and then calling Parse on the set
// with args.
//
...
// returns an error if i is not a pointer to struct, has an invalid flag tag,
...
// An optTag contains all the information extracted from a flag tag.
type optTag struct {
	name  string
	param string
	help  string
	...
	for {
		...
		if arg == "" || arg == "-" || arg == "--" {
			if param != "" {
				...
				return nil, fmt.Errorf("flag tag missing option name: %q", tag)
			}
			if o.name == "" {
				if next != "" {
					return nil, fmt.Errorf("flag tag missing option name: %q", tag)
		...
		if param != "" {
			if o.param != "" {
				return nil, fmt.Errorf("flag tag has multiple parameter names: %q", tag)
			}
			o.param = param
		}
		if o.name != "" {
			return nil, fmt.Errorf("flag tag has too many names: %q", tag)
		}
